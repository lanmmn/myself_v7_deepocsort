...
compile_model_dir="./compiled_model/"
LOG="./display_log"
...
if [ ! -d ${compile_model_dir} ];then
    echo ${compile_model_dir} "is not exist"
    mkdir ${compile_model_dir}
else
    echo ${compile_model_dir} "is exist"
fi

compile() {
  vai_c_xir \
  --xmodel      ${quantized_model_dir}/YoloBody_int.xmodel \
  --arch        $ARCH \
  --net_name    YoloBody_${TARGET} \
  --output_dir  ${compile_model_dir}
}
